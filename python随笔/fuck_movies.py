...
def get_city(url, i):
    ...
    cookies = {'cookie': 'bid=p-3qdZmRHRU; douban-fav-remind=1; ps=y; ue="1217096231@qq.com"; push_noty_num=0; push_doumail_num=0; ap=1;'}
    res = requests.get(url, headers=headers, cookies=cookies)
    if(res.status_code == 200):
        print("\n成功获取第{}个城市信息!".format(i))
    else:
        print("\n第{}个城市信息获取失败!".format(i))
...
def main(ID, pages):
    ...
    infos = {'name': name_list, 'city': city_list,
             'content': content_list, 'score': score_list, 'date': date_list}
    data = pd.DataFrame(
        infos, columns=['name', 'city', 'content', 'score', 'date'])
